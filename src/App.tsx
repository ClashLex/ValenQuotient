...
/**
 * Inner application component that manages layout, routing, and global state.
 */
function AppInner() {
  ...
  const {
    ...
    updateTrackerValue,
  } = useAppData(user, selectedCategory?.id, setSelectedCategory);
  ...
  /**
   * Generates user initials for avatar fallback display.
   */
  const initials = (() => {
  ...
  return (
    <div className="relative h-dvh max-h-dvh w-full bg-[#010828] text-cream font-sans selection:bg-neon selection:text-[#010828] flex flex-col md:flex-row overflow-hidden select-none">
      ...
      <div className="flex-grow flex flex-col h-full overflow-hidden relative min-w-0">
          ...
          <div className="flex items-center gap-2">
            ...
            {user ? (
              ...
              <>
                ...
                >
                  {user.photoURL ? (
                    <img src={user.photoURL} alt={`${user.displayName || 'User'} profile avatar`} className="w-5 h-5 rounded-full object-cover" />
                  ) : (
        ...
        {/* Main content area */}
        <main
          className="flex-grow overflow-hidden relative flex flex-col min-h-0 p-1 sm:p-3"
          role="main"
        >
          <div
            className={`flex-grow rounded-xl border border-white/5 bg-[#010828]/40 flex flex-col min-h-0 ${
              activeSection === 'trackers' ? 'overflow-hidden' : 'overflow-y-auto'
            }`}
          >
            <div
              className={`${activeSection === 'trackers' ? 'h-full flex flex-col min-h-0' : 'min-h-full'} p-2 sm:p-3`}
            >
        ...
        <footer className="hidden md:flex h-7 border-t border-white/5 bg-[#00041c] px-6 items-center justify-between text-cream/20 font-mono text-[7px] uppercase tracking-widest shrink-0 overflow-hidden">
          <span className="truncate">
            VERIFICATION SECURED BY GOLD STANDARD ACCREDITATION &amp; VOLUNTARY CARBON CONTRACTS
          </span>
        ...
        <nav
          ...
          className="md:hidden border-t border-white/5 bg-[#00051e]/95 backdrop-blur-md flex justify-around items-stretch shrink-0 z-30 pb-safe"
...
/**
 * Root application component wrapped in the AuthProvider.
 */
